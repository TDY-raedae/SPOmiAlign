...
class RegressionMatcher(nn.Module):
    ...
    # ===== 新增：傅里叶边缘权重计算辅助函数 =====
    def _compute_edge_weight(self, im_tensor, sigma, dist_decay):
        """
        im_tensor: (B, 3, H, W), normalized or [0,1]
        Returns: (B, 1, H, W) weight map in [0,1]
        """
        B, C, H, W = im_tensor.shape
        weights = []
        
        # 简单的反归一化/重缩放以便转为 uint8 图片进行处理
        # 假设输入经过了 ImageNet 标准化，这里简单做 MinMax 缩放即可恢复结构信息
        im_detached = im_tensor.detach().cpu()
        
        for b in range(B):
            img = im_detached[b].permute(1, 2, 0).numpy() # H,W,3
            ...
            H_filter = 1 - np.exp(-D2 / (2 * (sigma ** 2)))
            ...
            # 5. 二值化 + 距离变换
            _, edge_bin = cv2.threshold(edge_norm, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            edge_bin_inv = cv2.bitwise_not(edge_bin)
            dist = cv2.distanceTransform(edge_bin_inv, cv2.DIST_L2, 5)
            
            # 6. 指数衰减
            w_map = np.exp(-dist / dist_decay)
            # 归一化到 0-1
            w_map = (w_map - w_map.min()) / (w_map.max() - w_map.min() + 1e-8)
            
            weights.append(w_map)
            
        weights = np.stack(weights) # (B, H, W)
        weights_t = torch.from_numpy(weights).to(im_tensor.device).float().unsqueeze(1) # (B, 1, H, W)
        return weights_t
    ...
    def get_sorted_matches(self, warp, certainty, H, W):
        """
        获取按置信度降序排列的所有匹配点对 (不进行采样，返回全部)。
        
        Args:
            warp: (H_grid, W_grid, 4) 归一化坐标映射
            certainty: (H_grid, W_grid) 置信度
            H, W: 目标输出的像素分辨率 (通常是原图尺寸)
            
        Returns:
            kpts_A: (N, 2) 排序后的 A 图像素坐标
            kpts_B: (N, 2) 排序后的 B 图像素坐标
            sorted_cert: (N,) 排序后的置信度
        """
        # 1. 展平
        matches = warp.reshape(-1, 4) # (N, 4)
        cert = certainty.reshape(-1)  # (N,)
        
        # 2. 降序排序
        sorted_cert, indices = torch.sort(cert, descending=True)
        sorted_matches = matches[indices]
        
        # 3. 转换为像素坐标
        kpts_A, kpts_B = self.to_pixel_coordinates(sorted_matches, H, W, H, W)
        
        return kpts_A, kpts_B, sorted_cert
    ...
    def match(
        ...
        *args,
        ...
        device=None,
        # ===== 新增参数 =====
        edge_power=0.0,    # 边缘权重幂次，建议 1.0-3.0，0.0 表示不启用
        edge_sigma=2.0,    # 边缘检测 sigma
        edge_decay=5.0,    # 距离衰减
        no_upsample=False, # 强制不进行上采样
    ):
        self.train(False)
        if not batched:
        ...
        should_upsample = self.upsample_preds and not no_upsample
        ...
        if should_upsample and im_A_high_res is None and im_B_high_res is None:
        ...
        elif should_upsample and im_A_high_res is not None and im_B_high_res is not None:
            batch = {"im_A": im_A_high_res, "im_B": im_B_high_res, "corresps": finest_corresps}

        if should_upsample:
            scale_factor = math.sqrt(
                self.upsample_res[0] * self.upsample_res[1] / (560**2)
            )
            if symmetric:
        ...
        if edge_power > 0.0:
            # 计算 im_A 的边缘权重
            w_A = self._compute_edge_weight(batch["im_A"], edge_sigma, edge_decay)
            
            if symmetric:
                # 对称模式下，certainty 是 (2B, 1, H, W)，前 B 个是 A->B，后 B 个是 B->A
                w_B = self._compute_edge_weight(batch["im_B"], edge_sigma, edge_decay)
                
                # 在 batch 维度拼接以匹配 certainty
                w_combined = torch.cat([w_A, w_B], dim=0) 
                
                if w_combined.shape[-2:] != certainty.shape[-2:]:
                    w_combined = F.interpolate(w_combined, size=certainty.shape[-2:], mode='bilinear', align_corners=False)
                
                certainty = certainty * (w_combined ** edge_power)
            else:
                if w_A.shape[-2:] != certainty.shape[-2:]:
                    w_A = F.interpolate(w_A, size=certainty.shape[-2:], mode='bilinear', align_corners=False)
                certainty = certainty * (w_A ** edge_power)
        # ===================================
